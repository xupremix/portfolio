...
// Drop cargo housekeeping lines; keep rustc diagnostics verbatim and expand long type files.
function cleanOutput(text) {
  ...
  // Find long-type file notes generated by rustc and attach the un-truncated full types
  const longTypeRegex = /the full name for the type has been written to '([^']+)'/g;
  let match;
  const expandedTypes = [];
  while ((match = longTypeRegex.exec(text)) !== null) {
    const relFile = match[1];
    const fullPath = path.join(work, relFile);
    if (fs.existsSync(fullPath)) {
      const typeContent = fs.readFileSync(fullPath, 'utf8').trim();
      expandedTypes.push(typeContent);
    }
  }

  if (expandedTypes.length > 0) {
    cleaned += '\n\n# --- Expanded Full Type(s) from compiler ---\n' + expandedTypes.join('\n---\n');
  }
